...
class RabbitQueueCreateSchema(BaseModel):
    ...
    bindings: List[RabbitBindings] = Field(default_factory=list, description='Bindings of queue to exchange',
                                           min_items=1)
...
class RabbitQueueDetailSchema(BaseModel):
    uuid: UUID
    rabbit_server_id: int
    bindings: List[RabbitBindings] = Field(description='Bindings of queue to exchange', default_factory=list,
                                           min_items=1)
    starts_at: datetime
    expires_at: datetime
