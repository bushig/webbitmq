...
async def get_messages(
    request: Request, queue_id: str, page: int = 1, page_size: int = 50
):
...
async def get_queues_list(server_id: int, only_active: bool = False):
    ...
    queues_task = RabbitQueueListSchema.from_queryset(
        RabbitQueue.filter(rabbit_server__id=server_id)
        .order_by("-starts_at")
        .limit(250)
    )
...
async def create_queue(
    ...
    missing_exchanges = await check_if_all_exchanges_exist(
        rabbit_server, exchanges_list
    )
    if missing_exchanges:
        raise HTTPException(
            status_code=400, detail=f"Эксченжи не существуют: {missing_exchanges}"
        )
    ...
    queue_bindings = [
        QueueBindings(
            queue=queue, exchange_name=binding.exchange, routing_key=binding.routing_key
        )
...
async def get_queue_meta_data(queue_id: str):
    ...
    """
    rabbit_queue = await RabbitQueue.get(uuid=queue_id).prefetch_related(
        "bindings", "rabbit_server"
    )
    return rabbit_queue
