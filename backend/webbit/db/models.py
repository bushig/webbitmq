...
class RabbitServer(Model):
    ...
    username = fields.CharField(
        max_length=255
    )  # TODO: Think about encrypting secure data (SQLCipher?)
...
class RabbitQueue(Model):
    ...
    messages_type = fields.CharField(
        max_length=5, default="json"
    )  # how we should parse messages
...
RabbitServerSchema = pydantic_model_creator(
    RabbitServer, name="RabbitServer", exclude=("queues",)
)
...
RabbitQueueListSchema = pydantic_model_creator(
    RabbitQueue, name="RabbitQueueList", computed=("rabbit_server_id",)
)
